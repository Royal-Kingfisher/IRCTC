...
async function automateIRCTC() {
  ...
  await page.type('input[formcontrolname="userid"]', "RailYatriSeva", {
    delay: 150,
  });
  await page.type('input[formcontrolname="password"]', "Narela123@", {
    delay: 150,
  });

  const solveCaptcha = async (attempt = 1) => {
    if (attempt > 5) {
    ...
    console.log(`🔁 Attempt ${attempt} - Capturing CAPTCHA...`);
    const captchaImg = await page.$("img.captcha-img");
    const captchaPath = path.join(__dirname, "captcha.png");
    const processedPath = path.join(__dirname, "captcha-processed.png");

    await captchaImg.screenshot({ path: captchaPath });

    const image = await Jimp.read(captchaPath);
    await image.grayscale().contrast(1).writeAsync(processedPath);
    ...
    console.log("🧠 CAPTCHA Text:", cleaned || "(empty)");
    ...
    await page.type('input[formcontrolname="captcha"]', cleaned, {
      delay: 100,
    });
    ...
    const loginVisible = await page.$('input[formcontrolname="userid"]');
    if (loginVisible) {
      console.log("❌ Login failed. Retrying...");
      await page.click("img.captcha-img"); // refresh CAPTCHA
